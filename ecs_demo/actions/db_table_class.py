...
class MemberPoints(Base):
    ...
    __table_args__ = {'comment': '会员积分表'}

    user_id: Mapped[str] = mapped_column(String(50), primary_key=True, comment='用户ID')
    points: Mapped[int] = mapped_column(Integer, nullable=False, comment='当前积分')
    member_level: Mapped[str] = mapped_column(String(20), nullable=False, comment='会员等级')
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP, comment='更新时间')


class PointsRewards(Base):
    __tablename__ = 'points_rewards'
    __table_args__ = {'comment': '积分商品表'}

    reward_id: Mapped[str] = mapped_column(String(50), primary_key=True, comment='商品ID')
    reward_name: Mapped[str] = mapped_column(String(100), nullable=False, comment='商品名称')
    points_required: Mapped[int] = mapped_column(Integer, nullable=False, comment='所需积分')
    stock: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("'0'"), comment='库存')
    description: Mapped[Optional[str]] = mapped_column(String(200), comment='商品描述')


class PointsRedemptionRecords(Base):
    __tablename__ = 'points_redemption_records'
    __table_args__ = {'comment': '积分兑换记录表'}

    record_id: Mapped[str] = mapped_column(String(50), primary_key=True, comment='记录ID')
    user_id: Mapped[str] = mapped_column(String(50), nullable=False, comment='用户ID')
    reward_id: Mapped[str] = mapped_column(String(50), nullable=False, comment='商品ID')
    points_cost: Mapped[int] = mapped_column(Integer, nullable=False, comment='消耗积分')
    redeem_time: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP, comment='兑换时间')
    status: Mapped[Optional[str]] = mapped_column(String(20), server_default=text("'completed'"), comment='状态')
